...
Papertrail is a very simple "tail like" service for tailing distributed logs. We will use it to show how to configure the simplest possible static end point. Once you configure an account with Papertrail then you will be provided with a host and a port. Supposing that these values are `logs2.papertrailapp.com` and `38564` respectively:
...
#### Hosted solutions
You may want to investigate **Logstash** running with **Kibana** and **Elasticsearch** as an interesting hosted log analysis infrastructure. Of course they all provide syslog adapters, so can be easily used together with Contrail.
